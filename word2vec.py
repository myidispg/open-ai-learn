...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov  6 20:22:25 2018
...
with graph.as_default(), tf.device('/cpu:0'):
    ...
    loss = tf.reduce_mean(tf.nn.sampled_softmax_loss(weights=softmax_weigths, biases=softmax_biases, inputs=embed,
                                                     labels=train_labels, num_sampled=num_sampled, num_classes = vocabulary_size))
    
    # Optimizer.
    # Note: The optimizer will optimize the softmax weights AND the embeddings.
    # This is because the embeddings are defined as a variable quantity and the 
    # optimizer's minimze method will by default modify all the variable quantities
    # that contribute to the tensor it is passed.
    optimizer = tf.train.AdagradOptimizer(1.0).minimize(loss)
    
    # Compute the similarity between minibatch examples and all the embeddings.
